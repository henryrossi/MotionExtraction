...
int write_inverted_frame_yuv420p(VideoCodec *video_codec) {
        ...
        video_codec->inverted_data[0] = (uint8_t *)malloc(
            sizeof(uint8_t) * video_codec->frame->linesize[0] *
            video_codec->frame->height);
        video_codec->inverted_data[1] = (uint8_t *)malloc(
            sizeof(uint8_t) * video_codec->frame->linesize[1] *
            video_codec->frame->height);
        video_codec->inverted_data[2] = (uint8_t *)malloc(
            sizeof(uint8_t) * video_codec->frame->linesize[2] *
            video_codec->frame->height);
        ...
        if (video_codec->inverted_data[0] == NULL ||
            ...
            video_codec->inverted_data[2] == NULL) {
                fprintf(
                    stderr,
                    "ERROR:   Failed to allocate memory for inverted frames\n");
                return -1;
        }
        ...
        for (int y = 0; y < video_codec->frame->height; y++) {
                for (int x = 0; x < video_codec->frame->width; x++) {
                        video_codec->inverted_data
                            [0][y * video_codec->frame->linesize[0] + x] =
                            255 -
                            video_codec->frame
                                ->data[0]
                                      [y * video_codec->frame->linesize[0] + x];
                }
        }
        ...
        for (int y = 0; y < video_codec->frame->height / 2; y++) {
                for (int x = 0; x < video_codec->frame->width / 2; x++) {
                        video_codec->inverted_data
                            [1][y * video_codec->frame->linesize[1] + x] =
                            255 -
                            video_codec->frame
                                ->data[1]
                                      [y * video_codec->frame->linesize[1] + x];
                        video_codec->inverted_data
                            [2][y * video_codec->frame->linesize[2] + x] =
                            255 -
                            video_codec->frame
                                ->data[2]
                                      [y * video_codec->frame->linesize[2] + x];
                }
        }
        return 0;
}
...
int overlay_frames_yuv420p(VideoCodec *video_codec) {
        ...
        for (int y = 0; y < new_frame->height; y++) {
                for (int x = 0; x < new_frame->width; x++) {
                        ...
                        byte =
                            (video_codec
                                 ->inverted_data[0][y * new_frame->linesize[0] +
                                                    x] +
                             new_frame
                                 ->data[0][y * new_frame->linesize[0] + x]) /
                            2;
                        if (byte > 255) {
                                byte = 255;
                        }
        ...
        for (int y = 0; y < new_frame->height / 2; y++) {
                for (int x = 0; x < new_frame->width / 2; x++) {
                        ...
                        byte =
                            (video_codec
                                 ->inverted_data[1][y * new_frame->linesize[1] +
                                                    x] +
                             new_frame
                                 ->data[1][y * new_frame->linesize[1] + x]) /
                            2;
                        if (byte > 255) {
                                byte = 255;
                        }
                        ...
                        byte =
                            (video_codec
                                 ->inverted_data[2][y * new_frame->linesize[2] +
                                                    x] +
                             new_frame
                                 ->data[2][y * new_frame->linesize[2] + x]) /
                            2;
                        if (byte > 255) {
                                byte = 255;
                        }
